...
const ChatItem = ({userName, type, user}) => {
  ...
  return (
    type === "add"
      ?    
        <div className={`${styles.chatItem} ${styles.chatItemAdd}`} onClick={() => dispatch(addChat({chatUsers: [user.userName, authUser.userName], messages: []}))}>
          <h4>{userName}</h4>
        </div>
      : 
        <div className={`${styles.chatItem} ${isCurrentChat ? styles.currentChat : ""}`} onClick={() => {
